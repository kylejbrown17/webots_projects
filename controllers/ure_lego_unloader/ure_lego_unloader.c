...
int main(int argc, char **argv) {
  ...
  int counter = 0, i = 0;
  int state = WAITING;
  // const double target_positions[] = {0.0, -1.88, -2.14, -2.38, -1.51, 0.0};
  const double target_positions[] = {0.0, -1.78, -2.10, -2.62, -1.51, 0.0};
  double speed = 0.5;
  ...
  position_sensors[0] = wb_robot_get_device("shoulder_pan_joint_sensor");
  position_sensors[1] = wb_robot_get_device("shoulder_lift_joint_sensor");
  position_sensors[2] = wb_robot_get_device("elbow_joint_sensor");
  position_sensors[3] = wb_robot_get_device("wrist_1_joint_sensor");
  position_sensors[4] = wb_robot_get_device("wrist_2_joint_sensor");
  position_sensors[5] = wb_robot_get_device("wrist_3_joint_sensor");
  for (i = 0; i < 6; i++)
    ...
    if (counter <= 0) {
      switch (state) {
        ...
        case GRASPING:
          ...
          break;
        case ROTATING:
          if (wb_position_sensor_get_value(position_sensors[3]) < -2.3) {
            counter = 8;
        ...
        case RELEASING:
          ...
          break;
        case ROTATING_BACK:
          if (wb_position_sensor_get_value(position_sensors[3]) > -0.1) {
            state = WAITING;
            printf("Waiting can\n");
          }
          break;
